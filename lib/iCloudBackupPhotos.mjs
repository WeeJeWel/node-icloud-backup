...
export default class iCloudBackupPhotos {
  ...
  async createBackup() {
    ...
    while (true) {
      ...
      console.log(`📸 Scanning Photos: ${offset} - ${offset + length}`);
    ...
    for (const album of albums.values()) {
      ...
      this.log(`📒 Syncing Album '${album.name}'...`);
    ...
    // Download All Shared Albums, and symlink to All Photos
    for (const sharedAlbum of sharedAlbums.values()) {
      this.log(`📒 Syncing Shared Album '${sharedAlbum.name}'...`);

      await fse.ensureDir(path.join(this.filepath, sharedAlbum.name));

      const photos = await sharedAlbum.getPhotos();
      const photosFilenames = new Set();

      for (const photo of photos) {
        if (photo.record.recordType !== 'CPLMaster') continue;

        const fileExtension = atob(photo.record.fields.filenameEnc.value).split('.').pop().toLowerCase();
        const fileName = `${sha256(photo.record.recordName)}.${fileExtension}`;
        const filePathOriginal = path.join(this.filepath, 'All Photos', fileName);

        photosFilenames.add(fileName);

        const relativeFilepath = path.join(sharedAlbum.name, fileName);
        const absoluteFilepath = path.join(this.filepath, relativeFilepath);

        // Check if the original file exists
        const filePathOriginalExists = await fse.pathExists(filePathOriginal);
        if (!filePathOriginalExists) {
          // Somehow the photo is not in 'All Photos'.
          // Download the photo directly to the shared album
          const url = photo.record.fields.resOriginalRes.value?.downloadURL;
          if (!url) continue;
          ...
          try {
            ...
            // Set file modification time
            const dateModified = new Date(photo.record.fields.originalCreationDate.value);
            await fse.utimes(absoluteFilepath, dateModified, dateModified);
        ...
        // Check if the symlink already exists
        const symlinkExists = await fse.pathExists(path.join(this.filepath, sharedAlbum.name, fileName));
        if (symlinkExists) continue;
      ...
      // Remove symlinks that no longer exist in this album
      const files = await fse.readdir(path.join(this.filepath, sharedAlbum.name));
      for (const file of files) {
        const filePath = path.join(this.filepath, sharedAlbum.name, file);
